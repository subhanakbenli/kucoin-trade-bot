        ...
        for order in orders['data'].get('items', []):
            ...
            if (order['symbol'] == symbol and
                ...
                return 0, None  # Aynı emir mevcut
                  ...
                  order['side'] == side and
                  float(order['stopPrice']) < stop_price and
                  float(order['price'] or 0) < limit_price and
                  float(order['size']) == size):
...
def maliyet_hesapla(row, price):
    """Maliyet hesaplamasını yapar ve tabloyu günceller."""
    currency = get_currency_from_row(row)
    token_1, token_2 = currency.split("-")

    # Bakiyeleri al
    bakiye = ui.tableWidget.item(row, 1).text()
    token_1_bakiye, _ = parse_balance(bakiye)

    # Eski maliyeti al
    eski_maliyet = parse_old_maliyet(row)
    total_maliyet = token_1_bakiye * eski_maliyet

    # Yeni token_1 bakiyesini al
    token_1_yeni_bakiye = get_balance_for_token(token_1)
    if token_1_yeni_bakiye == 0:
        ui.statusBar.showMessage(f"{currency} tokeni bakiyesi 0 olduğu için maliyet hesaplanamadı", 5000)
        return 0

    # Yeni maliyeti hesapla
    yeni_maliyet = total_maliyet + (token_1_yeni_bakiye - token_1_bakiye) * price
    yeni_ortalama_maliyet = yeni_maliyet / token_1_yeni_bakiye

    # Maliyet ve bakiyeleri güncelle
    ui.tableWidget.setItem(row, 4, QTableWidgetItem(str(yeni_ortalama_maliyet)))
    
    token_2_yeni_bakiye = get_balance_for_token(token_2)
    token_1_yeni_bakiye = truncate_float(token_1_yeni_bakiye)
    token_2_yeni_bakiye = truncate_float(token_2_yeni_bakiye)
    ui.tableWidget.setItem(row, 1, QTableWidgetItem(f"{token_1_yeni_bakiye}/{token_2_yeni_bakiye}"))

    ui.statusBar.showMessage(f"{currency} tokeni maliyeti güncellendi", 5000)
