...
def check_existing_order(symbol, side, stop_price, limit_price, size):
    ...
    timestamp = str(int(time.time() * 1000))
    cursor.execute("SELECT priceIncrement FROM trade_data WHERE symbol = ?", (symbol,))
    price_increment = cursor.fetchone()[0]
    cursor.execute("SELECT baseIncrement FROM trade_data WHERE symbol = ?",(symbol,))
    base_increment = cursor.fetchone()[0]
    stop_price = round_to_increment(stop_price, price_increment)
    limit_price = round_to_increment(limit_price, price_increment)
    round_balance = round_to_increment(size, base_increment)
    ...
    if response.status_code == 200:
        ...
        for order in orders['data'].get('items', []):
            if (order['symbol'] == symbol and order['side'] == side and float(order['price']) == float(limit_price)):
                    if float(order['size']) == round_balance and float(order['stopPrice']) >= float(stop_price): # float(order['stopPrice']) == float(stop_price)
                        return 0, None  # Aynı emir mevcut
                    else:
                        # print(f"Emir miktarı güncellendi11: {order['size']} -> {size}")
                        return 1, order['id']
                    
            elif (order['symbol'] == symbol and order['side'] == side and round(float(order['price'] or 0), 2) != round(float(limit_price), 2)):
                # print(f"Emir fiyatı güncellendi: {order['price']} -> {limit_price}")
...
# def round_to_increment(price, price_increment):
#     """
...
def round_to_increment(value, increment):
    # Ondalık basamak sayısını belirlemek için increment'in basamak sayısını kullanır
    precision = abs(int(round(math.log10(increment))))
    # Aşağı doğru yuvarlama işlemi
    factor = 10 ** precision
...
def create_stop_loss_order(currency, row, balance, cost):
    ...
    price_increment = cursor.fetchone()[0]
    cursor.execute("SELECT baseIncrement FROM trade_data WHERE symbol = ?", (currency,))
    base_increment = cursor.fetchone()[0]

    round_cost = round_to_increment(cost, price_increment)  # cost değeri fiyat artışına göre yuvarlanır
    round_balance = round_to_increment(balance, base_increment)  # balance değeri base increment'a göre yuvarlanır
    
    price = get_price_from_row(row, 3)
    if price is None:
        handle_order_error(f"{currency} düşüş işlemi fiyatı alınamadı!")
        return
    ...
    if stoploss:
        ...
        round_stop_price = round_to_increment(stop_price, price_increment)
        print("currency",currency)
        print("Yuvarlanmış Stop Fiyatı:", round_stop_price)
        print("Yuvarlanmış Maliyet:", round_cost)
        print("Yuvarlanmış Bakiye:", round_balance)
        ...
        if round_cost >= price:
            print(f"{currency} satış marketi maliyetten düşük olamaz!")
            handle_order_error(f"{currency} düşüş işlemi maliyetten kaynaklı yapılamadı!")
            return

        order_response = place_order(currency, "sell", order_type="market", size=round_balance)
        print(order_response)
...
def dusus_tetiklendi(currency, row):
    ...
    cursor.execute("SELECT quoteIncrement FROM trade_data WHERE symbol = ?", (currency,))
    quote_increment = cursor.fetchone()[0]
    
    round_islem_hacmi = round_to_increment(islem_hacmi,quote_increment)
    round_birim_hacim = round_to_increment(birim_hacim,quote_increment)
    
    funds = min(round_islem_hacmi, round_birim_hacim)
    if funds < 0.1:
        # print(f"{currency} işlem hacmi yetersiz")
        return
...
def ortalama_hesapla(islemler, guncel_bakiye):
    ...
    for islem in islemler["items"]:
        if islem['side'] == 'buy':
            ...
            print(f"Alım: {miktar} x {fiyat} = {miktar * fiyat}")
        ...
        if truncate_float(toplam_miktar) >= truncate_float(guncel_bakiye):
...
def maliyet_guncelle(row):
    ...
    if ortalama_maliyet == 0:
        ui.statusBar.showMessage("Ortalama maliyet hesaplanamadı!", 6000)
        return
