...
def check_existing_order(symbol, side, stop_price, limit_price, size):
    ...
    stop_price = round_price_to_increment(stop_price, price_increment)
    limit_price = round_price_to_increment(limit_price, price_increment)
    ...
    if response.status_code == 200:
        ...
        for order in orders['data'].get('items', []):
            if (order['symbol'] == symbol and order['side'] == side and float(order['price']) == float(limit_price)):
                    ...
                    if float(order['size']) == float(size) and float(order['stopPrice']) >= float(stop_price): # Aynı emir mevcut
